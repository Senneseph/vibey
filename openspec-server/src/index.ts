...
interface Question {
  id: string;
  text: string;
  type: 'open' | 'choice' | 'boolean' | 'numeric';
  options?: string[];
  required: boolean;
  category: string;
}
...
`;
  ...
  async createProposal(description: string, context?: any): Promise<ChangeProposal> {
  ...
  async applyChange(changeId: string): Promise<{ success: boolean; message: string }> {
    try {
      ...
      const proposal: ChangeProposal = await fs.readJson(proposalPath);
  ...
  async archiveChange(changeId: string): Promise<{ success: boolean; message: string }> {
    try {
      const proposalPath = path.join(this.changesDir, changeId, 'proposal.json');
      if (!await fs.pathExists(proposalPath)) {
        return { success: false, message: `Change ${changeId} not found` };
  ...
  async startDiscovery(featureType: string, userInput: string): Promise<DiscoverySession> {
  ...
  async answerQuestion(sessionId: string, questionId: string, answer: string): Promise<Question | null> {
  ...
  async generateSpecFromDiscovery(sessionId: string): Promise<Specification> {
    const session = this.activeSessions.get(sessionId);
  ...
  async validateSpecification(spec: Specification): Promise<{ isValid: boolean; errors: string[] }> {
    const errors: string[] = [];
  ...
  private async extractRequirements(description: string): Promise<Requirement[]> {
  ...
  private async generateTasks(requirements: Requirement[]): Promise<Task[]> {
    const tasks: Task[] = [];
  ...
  private async generateSpecDeltas(requirements: Requirement[]): Promise<SpecDelta[]> {
  ...
  private generateSpecMarkdown(requirements: Requirement[]): string {
...
`;
  ...
  private estimateEffort(tasks: Task[]): string {
    ...
    if (taskCount <= 3) return 'Small (1-2 days)';
    if (taskCount <= 6) return 'Medium (3-5 days)';
    return 'Large (1+ weeks)';
  }

  private generateTasksMarkdown(tasks: Task[]): string {
...
`;
  ...
  private async applySpecDelta(delta: SpecDelta): Promise<void> {
  ...
  private async generateInitialQuestions(featureType: string): Promise<Question[]> {
  ...
  private async updateDiscoveryContext(session: DiscoverySession, questionId: string, answer: string): Promise<void> {
  ...
  private async generateNextQuestion(session: DiscoverySession): Promise<Question | null> {
    ...
    const unansweredQuestions = session.questions.filter(q => 
      !session.responses.some(r => r.questionId === q.id)
    );
  ...
  private generateOverviewFromDiscovery(session: DiscoverySession): string {
    const purposeResponse = session.responses.find(r => 
      session.questions.find(q => q.id === r.questionId)?.category === 'purpose'
    );

    return purposeResponse ? purposeResponse.answer : 'Feature overview based on discovery session';
  }

  private generateAcceptanceCriteria(session: DiscoverySession): AcceptanceCriterion[] {
  ...
  private generateTechnicalDetails(session: DiscoverySession): TechnicalDetail[] {
    return [{
  ...
  private identifyDependencies(session: DiscoverySession): Dependency[] {
  ...
  private parseSpecificationFromMarkdown(content: string, filePath: string): Specification {
  ...
  try {
    switch (name) {
      case "openspec_create_proposal": {
        const { description, context } = args as { description: string; context?: any };
      ...
      case "openspec_start_discovery": {
        const { featureType, userInput } = args as { featureType: string; userInput: string };
      ...
      case "openspec_answer_question": {
        const { sessionId, questionId, answer } = args as { 
          sessionId: string; 
          questionId: string; 
          answer: string 
        };
      ...
      case "openspec_validate_specification": {
        const { specification } = args as { specification: Specification };
