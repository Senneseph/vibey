...
// Track all tool calls and thinking updates for proper restoration
let toolCallHistory = [];
let thinkingUpdates = [];
...
function handleAgentUpdate(update) {
    // Store updates for potential restoration
    if (update.type === 'tool_start') {
        toolCallHistory.push(update);
    } else if (update.type === 'tool_end') {
        // Update the corresponding tool call in history
        const toolCall = toolCallHistory.find(t => t.id === update.id);
        if (toolCall) {
            toolCall.result = update.result;
            toolCall.error = update.error;
            toolCall.success = update.success;
        }
    } else if (update.type === 'thought' || update.type === 'thinking') {
        thinkingUpdates.push(update);
    }
