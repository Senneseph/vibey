...
class FilesystemMCPServer {
    ...
    async updateFile(filePath: string, content: string): Promise<{ success: boolean; message: string }> {
        try {
            const fullPath = path.join(this.workspaceRoot, filePath);
            await fs.writeFile(fullPath, content, 'utf-8');
    ...
    async listFiles(directoryPath: string): Promise<{ success: boolean; files?: string[]; message: string }> {
        try {
...
// Initialize Filesystem server with workspace root from environment or default to current directory
const workspaceRoot = process.env.VIBEY_WORKSPACE_ROOT || process.cwd();
const filesystemServer = new FilesystemMCPServer(workspaceRoot);
    ...
    try {
        switch (name) {
            ...
            case "filesystem_move_file": {
                const { sourcePath, destinationPath } = args as { sourcePath: string; destinationPath: string };
