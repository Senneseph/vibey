    ...
    return {
        ...
        execute: async (args) => {
            ...
            if (args.action === 'create_atomic_change') {
                if (!args.title) throw new Error('Title required for atomic change task');
            ...
            if (args.action === 'get_progress') {
                ...
                if (!progress) return `Task ${args.taskId} not found`;
                return JSON.stringify(progress, null, 2);
            }

            if (args.action === 'start_step') {
                if (!args.taskId || args.stepIndex === undefined) throw new Error('taskId and stepIndex required');
                const success = taskManager.startStep(args.taskId, args.stepIndex);
                if (!success) return `Failed to start step ${args.stepIndex} for task ${args.taskId}`;
