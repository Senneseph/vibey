...
const response_formatter_1 = require("./response_formatter");
class AgentOrchestrator {
    cancel() {
        if (this.abortController) {
            this.abortController.abort();
    ...
    async chat(userMessage, contextItems, onUpdate) {
        ...
        try {
            ...
            while (turns < MAX_TURNS) {
                ...
                if (!jsonMatch) {
                    // No tool call, just a text response.
                    const formattedResponse = response_formatter_1.ResponseFormatter.formatResponse(responseText, false);
                    this.context.history.push({ role: 'assistant', content: formattedResponse });
